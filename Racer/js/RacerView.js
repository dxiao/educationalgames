...
(function() {
  var Module, PuzzleSpecs, RacerCode, copy, createElement, createPuzzleOutput, createPuzzleTable, executePuzzle, extend, fillInCommandsColumn, finishExecution, getProcessToCssNumMapping, getRootElement, initPuzzle, onDragenter, printMemoryState, startDrag;
  ...
  fillInCommandsColumn = function(root, processes, ordering, cssMapping) {
    ...
    for (_i = 0, _len = commandCells.length; _i < _len; _i++) {
      ...
      _results.push($(cell).empty().append(createElement("div").addClass("dragHandle process" + cssMapping[processName]).text(command.label).data("commandOrder", {
        process: processName,
        order: commands[processName].length
      }).data("processName", processName).attr("draggable", true).append(createElement("span").addClass("glyphicon glyphicon-resize-vertical")).on("dragstart", startDrag)).off().on("dragenter", onDragenter));
    }
    return _results;
  };
